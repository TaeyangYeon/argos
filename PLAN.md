...
| Vision 타입별 이미지 | ALIGN("위치정렬")→ALIGN_OK≥1, INSP→OK≥1+NG≥1, 혼합→전부 | 알람 모달 출력 |
...
- ✅ **Step 1~32 완료**
- ✅ Step 33 완료 — BlobInspectionEngine
- ✅ Step 34 완료 — AnalysisWorker Align 단계 연동
...
- ✅ Step 41 완료 — FailureAnalyzer (FP/FN 오버레이 생성, AI 원인 분석)
- ✅ Step 42 완료 — FeasibilityAnalyzer (Rule-based 한계 판정, EL/DL 추천)
- ✅ Step 43~46 완료
- ✅ Step 47 완료 — FeasibilityTab 전체 구현 (기술 수준 배지, 점수 상세, AI 판단 근거, 추천 모델, Feature 요약, 의사결정 흐름도)
- ✅ Step 48 완료 — ArgosJSONExporter, ArgosPDFExporter, ArgosImageExporter, ExportDialog 구현
- ✅ Step 49 완료 — 대시보드 워크플로우 5단계 연동, 검사 목적 StatCard, 시그널 배선
- ✅ Step 50 완료 — UI 품질 개선, 디자인 토큰, 로딩 스피너, 빈 상태, 툴팁, E2E 테스트, PyInstaller 배포 패키징
- ✅ Step 51 완료 — 최종 회귀 테스트 1087개 통과, README/CHANGELOG/KNOWN_ISSUES/ROADMAP 문서화, v1.0.0 릴리스
...
| Phase 4 | Step 23~32 — 검사 목적 입력 UI + Align 통합 | ✅ 완료 |
| Phase 5 | Step 33~41 — Inspection 엔진 | ✅ 완료 |
| Phase 6 | Step 42~46 — 결과 뷰어 및 출력 | ✅ 완료 |
| Phase 7 | Step 35~44 — Inspection 엔진 | ✅ 완료 |
| Phase 8 | Step 45~49 — 결과 뷰어 및 출력 | ✅ 완료 |
| Phase 9 | Step 50~51 — 통합 완성 및 운영 품질 | ✅ 완료 |
...
## Phase 4: 검사 목적 입력 UI (Step 23~24) ✅ 완료
...
**완료 내용:**
- ui/pages/purpose_page.py: PurposePage 구현
  (검사 유형 exclusive 토글 5종, 예시 텍스트 자동 변경,
   PurposeValidator 연동, purpose_confirmed 시그널, load_purpose 복원)
- ui/components/sidebar.py: PageID.PURPOSE 추가, 🎯 검사 목적 메뉴 추가
- ui/main_window.py: PurposePage 등록 및 purpose_confirmed 시그널 연결
- tests/unit/test_purpose_page.py: 8개 테스트 추가 (confirm button size 포함)

**UI 버그 수정 이력 (향후 동일 패턴 참고용):**
- [버그1] 토글 버튼 텍스트 미표시
  원인: 전역 QSS(ui/style.py QPushButton 규칙)가 자식 위젯 stylesheet를 override.
  해결: ui/style.py의 전역 QPushButton 셀렉터를
        QPushButton:not(#typeBtn_...) 형태로 변경하여
        토글 버튼을 전역 규칙에서 제외 후 개별 setStyleSheet() 적용.

- [버그2] 창 축소 시 입력 위젯 겹침
  원인: PurposePage에 QScrollArea가 없어 창이 작아지면 위젯들이 겹침.
  해결: 전체 콘텐츠를 QScrollArea로 감싸고 setWidgetResizable(True) 적용.
        확정 버튼은 QScrollArea 외부 하단에 고정.

- [버그3] 목적 확정 버튼 세로 찌부
  원인: 버튼 setStyleSheet()에 margin: 16px 20px 설정으로
        setFixedHeight(52) 내부에서 실제 표시 높이가 20px으로 축소됨.
  해결: CSS margin 제거 후 layout.setContentsMargins()로 여백 처리.
...
**완료 내용:**
- ✅ AnalysisStep, StepStatus 열거형 및 AnalysisStepWidget 구현 (48px, 펄스 애니메이션 포함)
- ✅ AnalysisProgressPanel 구현 (4단계 진행률, reset_all, get_overall_progress)
- ✅ LogViewer 구현 (레벨별 색상, 자동 스크롤, 최대 500줄 제한)
- ✅ AnalysisWorker QThread 구현 (시그널 기반, cancel 지원, placeholder 단계들)
- ✅ AnalysisPage 구현 (Pre-flight 체크 5개 항목, 검사 목적 포함)
- ✅ MainWindow 시그널 연결 (purpose_confirmed, roi_confirmed)
- ✅ ImageStore.add_image() 테스트 편의 메서드 추가
- ✅ ui.style Colors/Fonts 클래스 추가로 컴포넌트 스타일 표준화
...
**완료 내용:**
- ui/pages/result_page.py: ResultPage 및 FeatureTab 클래스 구현 완료
- ui/main_window.py: analysis_complete 시그널 연결 완료  
- core/analyzers/feature_analyzer.py: FullFeatureAnalysis에 ai_summary, preprocessing_recommendations 필드 추가
- tests/unit/test_step26_result_page.py: 17개 테스트 케이스 작성 완료
- 히스토그램 통계 카드, 노이즈 배지 색상, 에지 분석, 캘리퍼 적합성, OK/NG 분리도 바, AI 요약, 전처리 권장사항 모두 구현
- 탭 자동 전환, 신호 와이어링, 테스트 커버리지 100% 달성
...
**완료 내용:**
- core/align/caliper_align.py: IAlignEngine 구현, Sobel 기반 에지 검출, ROI 지원, 4방향(Inward/Outward/Leftward/Rightward), 3가지 Polarity, 4가지 Condition, 4섹션 설계 문서(dict), BGR 오버레이 이미지 생성
- core/align/__init__.py: CaliperAlignEngine export 추가
- tests/unit/test_step28_caliper_align.py: 20개 테스트 케이스 (전원 통과)
...
**완료 내용:**
- core/align/feature_align.py: FeatureAlignEngine 구현 완료. ORB 기본 + SIFT 폴백, BFMatcher, Lowe's ratio test, 4섹션 설계 문서(dict), BGR 오버레이 이미지, ROI 지원
- core/align/contour_align.py: ContourAlignEngine 구현 완료. Canny 에지 + findContours, Hu Moments(matchShapes), 4섹션 설계 문서(dict), BGR 오버레이 이미지, ROI 지원
- core/align/blob_align.py: BlobAlignEngine 구현 완료. threshold+findContours 기반 밝은 Blob 검출, 면적/원형도/볼록도 필터, 4섹션 설계 문서(dict), BGR 오버레이 이미지, ROI 지원
- core/align/__init__.py: FeatureAlignEngine, ContourAlignEngine, BlobAlignEngine export 추가
- tests/unit/test_step29_align_engines.py: 15개 테스트 케이스 (전원 통과)
...
**완료 내용:**
- ui/pages/align_tab.py: AlignTab(QWidget) 신규 구현
  - _build_4section_table(design_doc): 섹션 헤더(강조)/서브키 행 렌더링, 문자열 design_doc(pattern_align 형식) 파싱 지원
  - _build_library_table(): 5열(개념/Keyence/Cognex/Halcon/MIL) × 4행(Search Length/Condition:Best/Inward/Projection Length) 정적 테이블
  - _build_fallback_log(design_doc): chain_stages_tried 포함 시 폴백 단계·성공 전략·실패 원인·AI 판단 여부 표시
  - load_result(result): 전략 요약 카드(strategy_name, score%, success badge), 기준점 카드, 오버레이 이미지(BGR→RGB888→QPixmap), None 시 플레이스홀더
- ui/pages/result_page.py: 플레이스홀더 탭 제거 → AlignTab 삽입, load_align_result(result) 메서드 추가(탭 인덱스 2로 전환)
- tests/unit/test_step31_align_tab.py: 29개 테스트 케이스 전원 통과
...
**목표:** Blob 감지 기반 Inspection 엔진 구현 및 검증

**완료 내용:**
- `core/models.py` — `InspectionCandidate` 데이터클래스 추가
- `core/inspection/blob_inspector.py` — `BlobInspectionEngine` 구현
- `tests/inspection/test_blob_inspector.py` — 16개 테스트 전체 통과

**핵심 클래스/메서드:**
- `BlobInspectionEngine.run(ok_images, ng_images, roi, purpose) -> list[InspectionCandidate]`
- 3종 Candidate: strict(A) / moderate(B) / loose(C)
- 4섹션 설계 문서: layout / parameters / result_calculation / rationale
- 라이브러리 매핑: Keyence / Cognex / Halcon / MIL
- 오버레이 이미지: `argos_overlays/{candidate_id}.png`
- 점수 공식: `score = ok_pass_rate × 0.5 + ng_detect_rate × 0.5`
...
- ALIGN_DESIGN 단계: AlignFallbackChain.run() 실행
- InspectionPurpose를 Align 전략에 전달
- 결과를 `_align_result` 및 `_results["align"]` 양쪽에 저장
- 단계별 시그널 정상 발행

**테스트:**
- `tests/workers/test_analysis_worker_align.py` — 14개 테스트 전체 통과
- 정상 흐름 / 빈 이미지 / 엔진 예외 / 시그널 발행 / 결과 저장 커버
...
**완료 내용:**
- `core/inspection/circular_caliper_inspector.py` — CircularCaliperInspectionEngine 구현
- CaliperCondition / CaliperPolarity / CaliperDirection Enum, CaliperCandidate dataclass
- N개 등간격 Caliper 배치, 방사형 Sobel 프로파일 에지 검출
- LSQ Circle Fit (algebraic Coope), 3σ 이상치 제거 + 재피팅
- 유효 Caliper < 8개 시 신뢰도 경고 생성
- 4섹션 설계 문서 + Keyence/Cognex/Halcon/MIL 라이브러리 매핑
- 3종 Candidate 생성 (caliper 8/12/16개, Inward/Outward)
- `tests/inspection/test_circular_caliper_inspector.py` — 20개 테스트 전체 통과
...
**완료 내용:**
- `core/inspection/linear_caliper_inspector.py` — LinearCaliperInspectionEngine 구현
- LinearCaliperDirection Enum (LEFTWARD/RIGHTWARD/UPWARD/DOWNWARD), LinearCaliperPreset dataclass
- CaliperCondition / CaliperPolarity Enum — circular_caliper_inspector에서 재사용
- N개 선형 등간격 Caliper 배치, 1D Sobel 프로파일 에지 검출 (양방향 스캔)
- 폭(width) / 직진도(straightness) / 평행도(parallelism) 측정 (np.polyfit degree=1)
- 3σ 이상치 제거 (20개 이상 포인트 필요 조건 수학적 보장)
- 유효 Caliper < 4개 시 신뢰도 경고 생성
- 4섹션 설계 문서 + Keyence/Cognex/Halcon/MIL 라이브러리 매핑
- 3종 Candidate 생성 (count=6/FIRST, count=10/BEST, count=8/opposite direction)
- `tests/inspection/test_linear_caliper_inspector.py` — 25개 테스트 전체 통과

**완료 조건:**
```
✅ 25개 테스트 통과
✅ 전체 suite 729개 통과 (무결성 유지)
✅ git commit
```
...
**완료 내용:**
- `core/inspection/pattern_inspector.py` — PatternInspectionEngine 구현
- PatternInspectionCandidate dataclass (threshold, min_area, blur_kernel, morph_kernel_size, candidate_id, design_doc, library_mapping)
- 3종 Candidate 생성: strict(threshold=30, min_area=50) / balanced(20, 30) / sensitive(10, 10)
- cv2.absdiff 기반 차분 계산, GaussianBlur 노이즈 억제, MorphOpen 소규모 블롭 제거
- defect_area / roi_area × 100 (%) — pass_score=5.0 % 기준 OK/NG 판정
- ok_pass_rate / ng_detection_rate / separation_score 계산
- 첫 번째 NG 이미지 위에 결함 영역 빨간색 오버레이 (BGR ndarray)
- 4섹션 설계 문서 (배치구조 / 개별파라미터 / 결과계산 / 선택근거)
- Keyence / Cognex / Halcon / MIL 라이브러리 매핑 (4개 개념)
- `tests/inspection/test_pattern_inspector.py` — 37개 테스트 전체 통과

**완료 조건:**
```
✅ 37개 테스트 통과
✅ 전체 suite 766개 통과 (무결성 유지)
✅ PROGRESS.md / PLAN.md 업데이트
```
...
**완료 내용:**
- `core/evaluation/feasibility_analyzer.py` — FeasibilityAnalyzer 구현
- score >= threshold → Rule-based 충분 판정
- score < threshold → AI 기반 EL/DL 결정 (analyze_safe 사용)
- AI 실패 시 로컬 휴리스틱 fallback (점수 갭 + 노이즈 수준 기반)
- inspection_type별 모델 추천 테이블 (EL/DL 각각)
- feature_summary (noise_level, edge_strength 등) 반영
- 29개 테스트 전체 통과
...
**완료 내용 (2026-04-15):**
- `tests/test_step43_inspection_e2e.py`: E2E 통합 테스트 10개 작성 및 통과
  - 전체 파이프라인 E2E, NG=0 시나리오 (2개), All-candidates-fail, Single engine success, Feasibility 임계값 (3개), FailureAnalyzer 오버레이, CandidateGenerator 규칙
- `ui/workers/analysis_worker.py`: INSPECTION_DESIGN 단계 (DynamicCandidateGenerator + InspectionOptimizer) 및 EVALUATION 단계 (FailureAnalyzer + FeasibilityAnalyzer) 구현
- NG=0 시나리오: WARNING 로그 + Inspection 스킵 처리
- 전체 테스트 897개 통과, 0 실패
...
- 탭: 요약 / Feature 분석 / Align 결과 / Inspection 결과 / Feasibility
...
**완료 내용 (2026-04-16):**
- `ui/pages/summary_tab.py`: SummaryTab(QWidget) 신규 구현 — 4개 카드 (검사 목적, 사용 전략, 최종 스코어, 권장 접근법)
- `ui/pages/inspection_tab.py`: InspectionTab(QWidget) 스켈레톤 — Best candidate 명/점수 표시
- `ui/pages/feasibility_tab.py`: FeasibilityTab(QWidget) 스켈레톤 — Level 배지, 근거 텍스트 표시
- `ui/pages/result_page.py`: 5-tab 구조 완성, load_all/load_summary/load_inspection_result/load_feasibility_result/load_feature_result 추가
- `ui/workers/analysis_worker.py`: analysis_complete 시그널이 aggregate dict 발행
- `ui/main_window.py`: load_all 단일 디스패처로 시그널 와이어링 통합
- `tests/unit/test_step44_result_viewer.py`: 10개 테스트 케이스 전원 통과
...
**완료 내용 (2026-04-17):**
- `ui/pages/inspection_tab.py`: InspectionTab 전체 구현 — Best Candidate 요약 카드 (점수 색상 임계: green≥70/yellow≥50/red<50), OK 통과율/NG 검출률 stat 표시, 분리 마진 경고, 4섹션 파라미터 테이블 (design_doc 자동 플래튼), 라이브러리 매핑 테이블 (vendor-keyed/concept-table/flat concept 3종 포맷 자동 감지), 오버레이 이미��� 뷰어 (ndarray→QPixmap 변환, 파일 경로 로드, 줌 인/아웃/리셋), Candidate 비교 테이블 (best row 하이라이트)
- `tests/unit/test_step45_inspection_tab.py`: 31개 테스트 케이스 전원 통과
- 기존 Step 44 테스트 10개 포함 전체 938개 테스트 통과
...
**Failure 케이스 뷰어 구현 완료 (Step 46에서 선행 구현):**
- FailureTab: FP/FN 요약 카드, 썸네일 그리드 (FP 빨강, FN 주황 테두리)
- FailureDetailDialog: 오버레이 뷰어 (줌 인/아웃/리셋), AI 원인 분석, 개선 방향
- ResultPage에 6번째 탭으로 통합, load_all 디스패처 연동
- 25개 테스트 — 완료일: 2026-04-18
...
**완료 내용 (2026-04-18):**
- FeasibilityTab 전체 구현 (스켈레톤 → 6섹션 완성)
  - 기술 수준 배지 (Rule-based/Edge Learning/Deep Learning 컬러 구분)
  - Score vs Threshold 프로그레스 바
  - 점수 상세 카드 (최고 점수, 임계값, 점수 차이, 해석 텍스트)
  - AI 판단 근거 카드 (quote-style 좌측 보더)
  - 추천 모델 테이블 (Rule-based 시 자동 숨김)
  - 이미지 특성 요약 (feature_summary 지표)
  - 의사결정 흐름도 (활성 경로 하이라이트)
- ResultPage.load_feasibility_result() 컨텍스트 전달 연동
- 47개 테스트 (test_step47_feasibility_tab.py)

**완료 조건:**
```
☑ pytest 통과 (Failure 케이스 뷰어)
☑ Feasibility 상세 표시 구현
☑ 47개 테스트 통과
```
...
**완료 내용 (2026-04-18):**
- `core/export/__init__.py` — 모듈 init
- `core/export/json_exporter.py` — ArgosJSONExporter (custom JSONEncoder, ndarray→메타데이터, dataclass 재귀 직렬화)
- `core/export/pdf_exporter.py` — ArgosPDFExporter (reportlab SimpleDocTemplate, 7섹션 리포트, 페이지 번호)
- `core/export/image_exporter.py` — ArgosImageExporter (align/inspection/failure 오버레이 일괄 저장, ndarray→PNG, 파일 경로→복사)
- `ui/dialogs/export_dialog.py` — ExportDialog (경로 선택, JSON/PDF/이미지 체크박스, 스타일링)
- `ui/pages/result_page.py` — 내보내기 버튼 추가, 결과 저장, 토스트 알림
- `tests/unit/test_step48_export.py` — 27개 테스트 (JSON 8, PDF 5, Image 6, Dialog 5, Integration 3)

**완료 조건:**
```
☑ pytest 통과 (1037 tests, 0 failures)
☑ JSON / PDF 직접 확인
☑ git commit
```
...
**완료 내용 (2026-04-19):**
- 대시보드 StatCard에 "검사 목적" 카드 추가 (미입력=gray / 확정됨=green + 유형 표시)
- WorkflowIndicator 4단계 → 5단계 업데이트 (검사 목적 입력 단계 삽입)
- DashboardPage에 on_purpose_confirmed / on_roi_confirmed / on_analysis_complete 슬롯 추가
- MainWindow 시그널 배선: purpose_confirmed → Dashboard, roi_confirmed → Dashboard, analysis_complete → Dashboard
- Dashboard refresh() 시 목적/ROI/분석 상태 반영, showEvent 자동 갱신
- update_from_store에 has_purpose 파라미터 추가 (하위 호환성 유지)
- 테스트 24개 작성 (test_step49_dashboard_workflow.py)
- 기존 테스트 3개 5단계 반영 업데이트

**완료 조건:**
```
☑ pytest 통과 (1063 passed)
☑ 워크플로우 5단계 직접 확인
☑ 전체 흐름 1회 완주 직접 확인
□ git commit
```
...
신규 파일:
- `ui/theme.py` — 디자인 토큰 중앙 관리 (Colors, Fonts, Spacing, FontSize, Radius, Tooltips, EmptyStateMessages)
- `ui/widgets/__init__.py`, `ui/widgets/loading_spinner.py` — 재사용 LoadingSpinner/LoadingOverlay 위젯
- `ui/widgets/empty_state.py` — 재사용 EmptyStateWidget
- `tests/e2e/__init__.py`, `tests/e2e/test_step50_e2e.py` — E2E 테스트 19개 (4 시나리오 + UI 검증)
- `argos.spec` — PyInstaller 스펙 파일 (macOS .app / Windows .exe / Linux)
- `scripts/build.sh`, `scripts/build.bat` — 빌드 스크립트
- `scripts/generate_assets.py` — 플레이스홀더 아이콘/스플래시 생성
- `assets/icon.png` (256×256), `assets/splash.png` (600×400)

수정 파일:
- `core/__init__.py` — `__version__ = "1.0.0"` 추가
- `main.py` — 버전 import
- `ui/main_window.py` — 타이틀바에 버전 표시
- `ui/pages/settings_page.py` — 슬라이더/입력 툴팁 8개 추가
- `ui/pages/result_page.py` — 빈 상태 메시지, 탭 툴팁 6개
- `ui/components/toolbar.py` — API 상태/버튼 툴팁
- `ui/components/workflow_indicator.py` — 5단계 툴팁
- `ui/dialogs/export_dialog.py` — 체크박스 툴팁 3개
- `tests/unit/test_main_window.py` — 버전 포함 타이틀 테스트 업데이트

E2E 테스트 (19개):
1. Full Inspection Workflow — 이미지→ROI→목적→분석→결과 6탭 로딩
2. Dashboard StatCard 갱신
3. Align-Only Workflow — Inspection 스킵 그레이스풀 처리
4. Preflight 위치정렬 검증
5. JSON/PDF Export 파일 생성
6. Export Dialog 툴팁 확인
7. Session Reset 전체 상태 초기화
8. WorkflowIndicator 리셋
9-19. UI 품질 검증 (버전, 빈 상태, 툴팁, 스피너, theme 토큰 등)

총 테스트: 1087개 통과

---

### Step 51 — 최종 검수 및 문서화 ✅ (2026-04-20 완료)
